		...
		<Card className="shadow-none gap-8 w-full border-none px-6 py-8 mb-24 rounded-xl">
			...
			<CardContent className="p-0 flex flex-col gap-6">
				{cartItems.map((cartItem) => (
					<ProductCard
						key={cartItem.id}
						className="p-0 flex-row justify-between items-center rounded-none"
					>
						<ProductImage
						...
						<div className="flex w-full items-start">
							<ProductContent className="p-0 gap-0 text-left items-start">
								<ProductTitle className="text-base font-normal tracking-normal">
									{cartItem.product.toUpperCase()}
								</ProductTitle>
								<span className="text-accent-foreground">{`$ ${cartItem.price}`}</span>
							</ProductContent>
							<CardFooter className="p-0 text-accent-foreground">{`x${cartItem.available}`}</CardFooter>
						</div>
					</ProductCard>
				))}
			</CardContent>
